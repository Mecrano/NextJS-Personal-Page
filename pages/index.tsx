  ...
  return (
    <>
      ...
      <section id="projects" className="mb-16 px-3">
        ...
        <div>{projects?.map(({ id, ...project }: Project, index: number) => <Project key={id} {...project} inverted={!!(index % 2)} />)}</div>
      </section>
      <section id="blog" className="mb-16 px-3">
        ...
        <div className="flex flex-wrap">{posts?.map(({ id, ...post }: Post, index: number) => <Post key={id} index={index + 1} actionText={t('blog.action')} {...post} />)}</div>
      </section>
    </>
  )
}
